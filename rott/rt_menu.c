...
CP_iteminfo CustomItems = {32, 52, 4, 0, 24, CustomMenuNames, mn_largefont };
...
CP_itemtype CustomMenu[] =
{
  ...
  {1, "custom4\0", 'C', (menuptr)CP_JoystickAxis}
};
...
CP_iteminfo ControlAxisSelectItems = { CONTROLSELECT_X, 17, 5, 0, 16, AxisNames, mn_tinyfont };
...
CP_MenuNames JoyBtnNames[] =
   ...
   "B15 \x9             "
#else
#error Please define a proper platform
#endif
   };

CP_iteminfo JoyBtnItems = { 19, 16, 16, 0, 11, JoyBtnNames, mn_8x8font };
...
CP_iteminfo JoyAxisItems = { 19, 48, 8, 0, 11, JoyAxisNames, mn_8x8font };

CP_itemtype JoyAxisMenu[] =
		{
	      { 2, "\0", 'B', (menuptr)DefineJoyAxis },
	      { 1, "\0", 'B', (menuptr)DefineJoyAxis },
	      { 1, "\0", 'B', (menuptr)DefineJoyAxis },
	      { 1, "\0", 'B', (menuptr)DefineJoyAxis },
	      { 1, "\0", 'B', (menuptr)DefineJoyAxis },
	      { 1, "\0", 'B', (menuptr)DefineJoyAxis },
...
void DrawControlAxisSelect
   ...
   SetAlternateMenuBuf();
   ClearMenuBuf();
   SetMenuTitle ("Select Axis Function");

   MN_GetCursorLocation( &ControlAxisSelectItems, &ControlSelectMenu[ 0 ] );
   DrawMenu( &ControlAxisSelectItems, &ControlSelectMenu[ 0 ] );
...
void DefineJoyAxis
   ...
   MN_GetActive( &ControlAxisSelectItems, &ControlSelectMenu[ 0 ],
      axisjoy[ button ], controlorder );

   DrawControlAxisSelect();

   which = HandleMenu( &ControlAxisSelectItems, &ControlSelectMenu[ 0 ], NULL );
   if ( which != -1 )
      {
      axisjoy[ button ] = which-1;
      }
...
void DrawCustomJoystick
   ...
   for( i = 0; i < JoyBtnItems.amount; i++ )
      {
      num = 0;
      button = buttonjoy[ i ];
...
#elif USE_SDL
   int s, n_jc = IN_JoyButtonCount ();
   for (s=n_jc; s < JoyBtnItems.amount; s++)
  	 {
  		 JoyBtnMenu[s].active = CP_Inactive;
  	 }
#endif
...
void DrawCustomJoystickAxis
   ...
   SetAlternateMenuBuf();
   ClearMenuBuf();
   SetMenuTitle ("Customize Joystick Axis");

#if USE_SDL
   int s, n_jc = IN_JoyAxisCount ();
   for (s=n_jc; s < JoyAxisItems.amount; s++)
  	 {
  		 JoyAxisMenu[s].active = CP_Inactive;
  	 }
#endif
   ...
   for( i = 0; i < 8; i++ )
      ...
      strcpy( &JoyAxisNames[ i ][ JOYBTNINDEX ], AxisNames[ num ] );
      }

   if ( JoyAxisMenu[ JoyAxisItems.curpos ].active == CP_Inactive )
      {
      MN_GetCursorLocation( &JoyAxisItems, &JoyAxisMenu[ 0 ] );
      }
   else
      {
      JoyAxisMenu[ JoyAxisItems.curpos ].active = CP_CursorLocation;
      }

   DrawMenu( &JoyAxisItems, &JoyAxisMenu[ 0 ] );
...
void CP_JoystickAxis
   ...
   do
      {
      DrawCustomJoystickAxis();
      which = HandleMenu( &JoyAxisItems, &JoyAxisMenu[ 0 ], NULL );
      }
   while( which >= 0 );
...
#if USE_SDL
   //You usually can't achieve full value on analog joysticks, lets use lower values for the max/min values.
   joyxmin = 25;
   joyxmax = 230;
   joyymin = 25;
   joyymax = 230;
   ...
   for (i = 0; i < ExtOptionsItems.amount; i++)
      if (ExtOptionsMenu[i].active != CP_Active3)
      {
         ...
         switch (i)
         {
            ...
            case 5: if (playerautoaim == 1) on = 1; break;
         }
