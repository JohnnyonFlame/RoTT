...
//   int temp;
   ...
   if (version == ROTTVERSION)
   {
      ...
      for (i=0;i<16;i++)
      	{
      		char c[16];
      		sprintf(c, "JoyButton%i", i);

      		ReadInt(c, &buttonjoy[i]);
      	}
#endif
      ...
      ReadInt ("AimAssist",   &playerautoaim);
...
//   WriteParameter(file,"SpaceballEnabled ",spaceballenabled);
      ...
      for (i=0;i<16;i++)
      	{
      		...
      		WriteParameter (file, c, buttonjoy[i]);
      	}
#endif
		...
		for (i=0;i<8;i++)
			{
				...
				WriteParameter (file, c, axisjoy[i]);
			}
   ...
   WriteParameter (file,"JoyMinY          ", joyymin);
	 ...
	 WriteParameter (file,"AimAssist        ", playerautoaim);
