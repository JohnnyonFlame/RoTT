...
void IN_GetJoyAbs (word joy, word *xp, word *yp)
{
   ...
   {
	   Joy_x = (SDL_JoystickGetAxis (sdl_joysticks[joy], 0)>>8)+128;
	   Joy_y = (SDL_JoystickGetAxis (sdl_joysticks[joy], 1)>>8)+128;
   } else {
	   Joy_x = Joy_y = 0;
   }
#endif
...
void IN_GetJoyAxisAbs (word joy, int axis, word *value)
{
#ifdef USE_SDL
   if (joy < sdl_total_sticks)
   {
	   Joy_x = (SDL_JoystickGetAxis (sdl_joysticks[joy], axis)>>8)+128;
   } else {
	   Joy_x = 0;
   }
   *value = Joy_x;
...
void INL_GetJoyAxisDelta (word joy, int axis, int *value)
{
   word        ax;
   JoystickDef *def;
   static longword lasttime;

   IN_GetJoyAxisAbs (joy, axis, &ax);
   def = JoyDefs + joy;

   if (ax < def->threshMinX)
   {
      if (ax < def->joyMinX)
         ax = def->joyMinX;

      ax = -(ax - def->threshMinX);
      ax *= def->joyMultXL;
      ax >>= JoyScaleShift;
      *value = (ax > 127)? -127 : -ax;
   }
   else if (ax > def->threshMaxX)
   {
      if (ax > def->joyMaxX)
         ax = def->joyMaxX;

      ax = ax - def->threshMaxX;
      ax *= def->joyMultXH;
      ax >>= JoyScaleShift;
      *value = (ax > 127)? 127 : ax;
   }
...
int IN_JoyButtonCount (void)
{
	return SDL_JoystickNumButtons(sdl_joysticks[joystickport]);
}

int IN_JoyAxisCount (void)
{
	return SDL_JoystickNumAxes(sdl_joysticks[joystickport]);
}

#else

//Doubt this will be called when USE_SDL is not set, but we gotta make sure.
int IN_JoyButtonCount (void)
{
	return 4;
}
...
#if USE_SDL
   int i;
   for (i=0;i<IN_JoyButtonCount();i++)
  	 {
  		 joybits += SDL_JoystickGetButton(sdl_joysticks[joystickport], i)<<i;
  	 }
