...
int      buttonjoy[16] = {
		bt_attack, bt_strafe, bt_run, bt_use,
...
void PollJoystickMove (void)
{
   ...
   for (i=0;i<IN_JoyAxisCount();i++)
  	 {
  		 if (axisjoy[i] == ax_ignore)
  			 continue;

				playertype * pstate;

  		 int value;
  		 INL_GetJoyAxisDelta(joystickport, i, &value);
  		 if (value)
  			 {
				 switch (axisjoy[i])
				 {
				 case ax_turn:
					 JX = ((-value)<<13)+((-value)<<11);
					 JX = FixedMul(JX,sensitivity_scalar[mouseadjustment]*MOUSE_RY_SENSITIVITY_SCALE);
					 break;

				 case ax_strafe:
					 JS = ((-value)<<13)+((-value)<<11);
					 break;

				 case ax_lookver:
						pstate=&PLAYERSTATE[consoleplayer];
						pstate->horizon -= ((value) * (2*sensitivity_scalar[mouseadjustment]))/32*inverse_mouse;
					 break;
...
void PollMove (void)
{
   ...
   if ((locplayerstate->NETCAPTURED == 1) && (!locplayerstate->HASKNIFE))
   {
                                  ...
                                  (leftmom > (NETMOM * 2)))
      {
         ...
         NewState(player, &s_player);
			...
			locplayerstate->weaponheight = WEAPONS[locplayerstate->weapon].screenheight;
      }
   }
   ...
   if (JS)
       ...
       controlbuf[0] += -(FixedMul (JS, costable[angle]));
       controlbuf[1] += FixedMul (JS, sintable[angle]);
  	 }
